...
class Service:
    ...
    def __add_to_db(
        self, func_name: str, nr_params: int, params_name: str, params_value: str
    ) -> None:
        connection = None  # it can be unbound in finally
        try:
            ...
            cursor.execute(
                query,
                (func_name, nr_params, params_name, params_value, str(datetime.now())),
            )
            connection.commit()
            ...
            print(
                "Error occured when trying to connect with the database for adding entry...\n",
                err,
            )
        finally:
            if connection:
    ...
    def cmmmc(self, *, n: int, m: int) -> float:
